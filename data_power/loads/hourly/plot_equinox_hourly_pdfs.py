...
load_data_low = np.genfromtxt("data/load_equinox_hourly_data_low.txt")
load_data_high = np.genfromtxt("data/load_equinox_hourly_data_high.txt")
x = np.arange(50., 110.)
...
# plt.plot(x, weibull(x, weib_scale, weib_shape) * sumcount_high, label="weibull, AIC 185.34")

norm_mu = 91.5625
norm_sigma = 5.030765
plt.plot(x, normal(x, norm_mu, norm_sigma) * sumcount_high, color="teal", label="normal, AIC 198.62")

norm_mu = 66.5
norm_sigma = 5.19615
plt.plot(x, normal(x, norm_mu, norm_sigma) * sumcount_low, color="red", label="normal, AIC 103.06")

# lnorm_mu = 4.18353
# lnorm_sigma = 0.109933
# plt.plot(x, lognormal(x, lnorm_mu, lnorm_sigma) * sumcount_low, label="log-normal, AIC 133.74")
...
plt.title("Equinox", fontsize="x-large")
